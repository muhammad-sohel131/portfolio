...
const CodingProfiles = () => {
  ...
  return (
    <section id="coding-profiles" className="py-20" style={{ backgroundColor: 'var(--color-background)' }}>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {platforms.map((platform, idx) => (
            ...
            >
              ...
              <div className="relative rounded-xl shadow-lg overflow-hidden hover:shadow-2xl transition-all duration-300" style={{ backgroundColor: 'var(--surface)', border: '1px solid var(--color-border)' }}>
                ...
                <div className="p-6">
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    <div className="flex-1">
                        <h3 className="text-xl font-bold" style={{ color: 'var(--color-foreground)' }}>{platform.name}</h3>
                        <p className="text-sm font-mono" style={{ color: 'var(--color-muted)' }}>@{platform.username}</p>
                      </div>
                      <i className="fas fa-external-link-alt transition" style={{ color: 'var(--color-muted)' }}></i>
                  </div>
                    ...
                    <div className="space-y-2 mt-4">
                      {Object.entries(platform.stats).map(([key, value], i) => (
                        <div key={i} className="flex justify-between items-center text-sm">
                          <span className="capitalize font-mono" style={{ color: 'var(--color-muted)' }}>{key}:</span>
                          <span className="font-semibold" style={{ color: 'var(--color-foreground)' }}>{value}</span>
                        </div>
                      ))}
                    </div>
                  )}
                  
                  <button className="mt-4 w-full py-2 rounded-lg font-mono text-sm hover:bg-gradient-to-r hover:from-blue-600 hover:to-indigo-600 hover:text-white transition-all duration-300" style={{ backgroundColor: 'var(--color-secondary)', color: 'var(--color-foreground)' }}>
                    Visit Profile →
                  </button>
                </div>
              </div>
